...
public class MenuActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!sharedPreferences.getBoolean("last_root_check", true))
            findViewById(R.id.no_root).setVisibility(View.VISIBLE);
        ...
        findViewById(R.id.no_root).setOnTouchListener(new TouchScaleListener());
        findViewById(R.id.preview).setOnTouchListener(new TouchScaleListener());
        findViewById(R.id.update_log).setOnTouchListener(new TouchScaleListener());
        findViewById(R.id.exit_btn).setOnTouchListener(new TouchScaleListener());
    }
    ...
    private void save() {
        try {
            ...
            if (!uiScaleInput.getText().toString().isEmpty()) {
                ...
                if (dpiTimes >= 0.3F && dpiTimes <= 3.0F)
                    sharedPrefercences_editor.putFloat("dpi", dpiTimes).commit();
            ...
            if (((MaterialRadioButton) findViewById(R.id.type_pm)).isChecked())
                sharedPrefercences_editor.putInt("set_install_type", 0);
            else if (((MaterialRadioButton) findViewById(R.id.type_copy)).isChecked())
                sharedPrefercences_editor.putInt("set_install_type", 1);
