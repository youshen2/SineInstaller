...
public class InstallerActivity extends BaseActivity implements ICommanderCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.install_btn).setOnTouchListener(new TouchScaleListener());
        findViewById(R.id.cancel_btn).setOnTouchListener(new TouchScaleListener());
        ...
        if (apkInfo == null || TextUtils.isEmpty(apkInfo.getPackageName())) {
            ...
            if (apkInfo.isHasInstalledApp()) {
                ...
                if (apkInfo.getVersionCode() < apkInfo.getInstalledVersionCode())
                    Toast.makeText(this, "即将降级安装", Toast.LENGTH_LONG).show();
    ...
    public void onApkPreInstall(ApkInfo apkInfo) {
        ...
        is_installing = true;
        if (sharedPreferences.getBoolean("set_show_log", true))
            log_view_card.setVisibility(View.VISIBLE);
    ...
    public void onApkInstalled(ApkInfo apkInfo, int resultCode) {
        ...
        if (resultCode == 0) {
            ...
            if (install_log.contains("INSTALL_FAILED_INSUFFICIENT_STORAGE"))
                dialog_content = "存储空间不足";
            else if (install_log.contains("INSTALL_PARSE_FAILED_INCONSISTENT_CERTIFICATES"))
                dialog_content = "安装包签名与已安装应用的不一致";
            else if (install_log.contains("INSTALL_FAILED_VERSION_DOWNGRADE"))
                dialog_content = "降级安装失败";
            else if (install_log.contains("INSTALL_FAILED_CPU_ABI_INCOMPATIBLE"))
                dialog_content = "该安装包不支持本设备";
            else if (install_log.contains("INSTALL_FAILED_NEWER_SDK"))
                dialog_content = "您的安卓版本过高";
            else if (install_log.contains("INSTALL_FAILED_UPDATE_INCOMPATIBLE"))
                dialog_content = "您可能需要先卸载掉已安装的版本";
            else if (install_log.contains("INSTALL_FAILED_TEST_ONLY"))
                dialog_content = "该安装包为TestOnly的测试包，无法进行安装";
